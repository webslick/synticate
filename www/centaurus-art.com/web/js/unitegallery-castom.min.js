// Unite Gallery, Version: 1.7.3 - Castom Chintamani
...
function onBeforeReqestItems(){g_objTilesWrapper.hide();if(g_objPreloader)
g_objPreloader.show();var preloaderSize=g_functions.getElementSize(g_objPreloader);var galleryHeight=preloaderSize.bottom+30;g_objWrapper.height(galleryHeight);}
...
jQuery(document).ready(function(){var apiGallery=jQuery("#prod_grid").unitegallery({gallery_width:"100%",gallery_min_width:300,tiles_type:"nested",tiles_nested_optimal_tile_width:400,tiles_space_between_cols:20,tile_show_link_icon:true,tile_enable_textpanel:true,tile_min_columns:2,tile_link_newpage:false,tile_space_between_icons:20,tiles_justified_row_height:400});jQuery('#prod_grid .ug-thumb-wrapper').each(function(i,el){var element=apiGallery.getItem(i);if(element.link==''){jQuery(el).removeClass('ug-tile-clickable');jQuery(el).find('.ug-thumb-overlay').remove();jQuery(el).find('.ug-thumb-image').css('cursor','default');}
if(element.title==''){jQuery(el).find('.ug-textpanel').html('');}});});jQuery(document).ready(function(){var apiGallery=jQuery("#brend_gallery").unitegallery({tiles_type:"nested",tiles_nested_optimal_tile_width:400,tiles_space_between_cols:20,tile_show_link_icon:true,tile_min_columns:2,tile_enable_textpanel:true,tile_link_newpage:false,tiles_nested_optimal_tile_width:400});jQuery('#brend_gallery .ug-thumb-wrapper').each(function(i,el){var element=apiGallery.getItem(i);if(element.link==''){jQuery(el).removeClass('ug-tile-clickable');jQuery(el).find('.ug-thumb-overlay').remove();jQuery(el).find('.ug-thumb-image').css('cursor','default');}
if(element.title==''){jQuery(el).find('.ug-textpanel').html('');}});});