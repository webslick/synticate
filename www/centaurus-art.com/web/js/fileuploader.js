...
qq.FileUploader = function(o){
    this._options = {
        ...
        template: '<div class="qq-uploader">' + 
                '<div class="qq-upload-drop-area"><span>Перетащите файлы на этот блок</span></div>' +
                '<div class="qq-upload-button" id="id-qq-upload-button"><a class="btn"><i class="glyphicon glyphicon-picture"></i></a></div>' +
                '<ul class="qq-upload-list" style=\"display:\"></ul>' + 
             '</div>',
        ...
        fileTemplate: '<li>' +
                ...
                '<span class="qq-upload-failed-text" style=\"display:none;\">Ошибка</span>' +
            '</li>',
...
qq.FileUploader.prototype = {
    ...
    _setupDragDrop: function(){
		...
		dropArea.nextSibling.style.display = 'block';
        ...
        qq.attach(document, 'dragover', function(e){
            if (isValidDrag(e)){
                ...
                if (dropArea == e.target || qq.contains(dropArea,e.target)){
					...
					dropArea.nextSibling.style.display = 'none';
    ...
    _addToList: function(id, fileName){
		...
		qq.setText(fileElement, this._formatFileName(''));
        this._getElement(item, 'file').id = fileName;
		this._getElement(item, 'size').style.display = 'none';
		this._getElement(item, 'cancel').style.display = 'none';
    ...
    _updateProgress: function(id, loaded, total){
        ...
        if (loaded != total){
			...
			ww = Math.round(loaded / total * 100) * 2;
			text = "<span class=\"qq-uploader-progress\"><span style=\"width:"+ww+"px\"></span></span>";
        } else {                                   
            //text = this._formatSize(total);
			text = "<span class=\"qq-uploader-progress-active\"></span>";
        }          
        
        qq.setHTML(size, text);
    },
...
function startFileUploader(params){
	if(!params) params = {};
	if(!params['table']){ params['table'] = 'tasks'; }
	if(!params['callback']) params['callback'] = 'test';
	var paramss = {'table':params['table'],'callback':params['callback']};
	//console.log("PARAMSS:\n"+JSON.stringify(paramss));
	var uploader = new qq.FileUploader({
		// pass the dom node (ex. $(selector)[0] for jQuery users)
		element: document.querySelector('#file-uploader').getElementsByTagName('div')[0],
		action: qqUploadAction,
		params: paramss,
		onComplete: function(id, fileName, responseJSON){
			//console.log("upload: "+fileName+"\n-------\nresponseJSON: "+JSON.stringify(responseJSON));
			var myParams = {};
			for(var j in responseJSON){
				myParams[j] = responseJSON[j];
			}
			eval(responseJSON.callback+"(myParams)");
			
			var obj = document.getElementById(fileName).parentNode;
			obj.parentNode.removeChild(obj);
		//	if(typeof currentFile!='undefined'){
		//		//getFilemanager();
		//	}else{
		//		//insertImgModule(responseJSON.fileName);
		//	}
		//	if(typeof responseJSON.callback){
		//		
		//	}
		}
	});
